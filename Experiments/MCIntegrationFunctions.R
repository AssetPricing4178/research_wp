...
compareMcIntegration <- function(nseries, nvalues, nDim, fDim, vectorLowerLimit, 
  ...
  #print estimates
  for(i in 1:nseries){
    print(paste(vectorRNG[i],"-Estimate:",matrixOfSeries[nvalues,i])    )
  }
  ...
  for(i in 1:nseries){
    hist(matrixOfSeries[,i], main = vectorRng[i], breaks = round(nvalues/2),
         xlim = c(matrixOfSeries[nvalues,1] -0.01, 
         matrixOfSeries[nvalues,1] +0.01), xlab = "Estimate")
  }
}